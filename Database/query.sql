...
create view view_permission as (
 ...
 from table_permission
   join table_staff as staff on permission_staffId = staff.staff_id
   join table_role on permission_roleId = role_id
   join table_staff as createStaff on permission_createStaffId = createStaff.staff_id
   left outer join table_staff as updateStaff on permission_updateStaffId = updateStaff.staff_id
...
create view view_session as (
 ...
 from table_session
   join table_staff as doctor on session_doctorId = doctor.staff_id
...
insert into table_role (role_name) values ('Quản trị');
insert into table_role (role_name) values ('Bác sĩ');
insert into table_role (role_name) values ('Kỹ thuật viên');
...
insert into table_session(session_date, session_doctorId, session_technicianId, session_createStaffId) values(convert(now(), date), 2, 6, 1);

insert into table_group(group_year, group_total) values (2018, 0);
insert into table_group(group_year, group_total) values (2019, 0);
insert into table_group(group_year, group_total) values (2020, 0);